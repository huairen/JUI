...
void AttriImage(JuiControl *control, int ctrlClass, int param, char* value)
{
	...
	else if (ctrlClass & CTRL_CLASS_BUTTON)
	{
		...
		case IMG_EFFECT_NORMAL:
			static_cast<JuiButton*>(control)->LoadNormalImage(value);
			break;
		case IMG_EFFECT_HIGHLIGHT:
			static_cast<JuiButton*>(control)->LoadHighlightImage(value);
			break;
		case IMG_EFFECT_PRESS:
			static_cast<JuiButton*>(control)->LoadPressImage(value);
			break;
		}
	}
...
struct ControlAttribute
{
	...
	int nameLen;
	int objClass;
	void (*attri)(JuiControl*,int,int,char*);
	int param;
};

ControlAttribute attriTable[] = {
	{"position", 8, CTRL_CLASS_BASE, AttriPoint, PT_EFFECT_POSITION},
	{"size", 4, CTRL_CLASS_BASE, AttriPoint,PT_EFFECT_SIZE},
	{"minSize", 7, CTRL_CLASS_BASE, AttriPoint, PT_EFFECT_MINSIZE},
	{"maxSize", 7, CTRL_CLASS_BASE, AttriPoint, PT_EFFECT_MAXSIZE},
	{"horizAlign", 10, CTRL_CLASS_BASE, AttriHorizAlign, 0},
	{"vertAlign", 9, CTRL_CLASS_BASE, AttriVertAlign, 0},
	{"visible", 7, CTRL_CLASS_BASE, AttriFlag, CTRL_FLAG_VISIBLE},
	{"enable", 6, CTRL_CLASS_BASE, AttriFlag, CTRL_FLAG_ENABLE},
	{"readonly", 8, CTRL_CLASS_BASE, AttriFlag, CTRL_FLAG_READONLY},
	{"mouseEnable", 11, CTRL_CLASS_BASE, AttriFlag, CTRL_FLAG_MOUSE_ENABLE},
	...
	{"stateOn", 7, CTRL_CLASS_RADIO_BUTTON, AttriStateOn, 0},
};
...
bool JuiCreater::SetAttribute( JuiControl *control, int ctrlClass, const char *name, char *value )
{
	...
	for (int i = 0; i < attriNum; i++)
	{
		...
		{
			attri.attri(control, ctrlClass, attri.param, value);
			break;
		}
	}
